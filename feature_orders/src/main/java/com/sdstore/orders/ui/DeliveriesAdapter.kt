...
import com.sdstore.core.R as coreR // Core module ke resources ke liye alias
...
class DeliveriesAdapter(private val onOrderClick: (Order) -> Unit) :
    ...
    inner class DeliveryViewHolder(private val binding: ItemDeliveryBinding) : RecyclerView.ViewHolder(binding.root) {
        fun bind(order: Order) {
            ...
            // Status Icon aur Color set karein
            when (order.status.lowercase(Locale.getDefault())) {
                ...
                else -> { // Pending ya koi aur status
                    binding.deliveryStatusIcon.setImageResource(R.drawable.ic_scheduled) // feature_orders ka resource
                    binding.deliveryStatusIcon.clearColorFilter()
                }
            }

            binding.tvDeliveryId.text = context.getString(R.string.order_id_prefix, order.orderId)
            ...
            } ?: context.getString(R.string.date_not_available)
